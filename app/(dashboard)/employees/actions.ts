...
import type { EmployeeFormData, VacationSummary } from "@/types"
...
export async function createEmployee(data: EmployeeFormData) {
  try {
    ...
    if (settings) {
      ...
      await (supabase.from("vacation_balance") as any)
        .insert({
          employee_id: employee.id,
          year: currentYear,
          days_from_previous_year: data.days_from_previous_year ?? 0,
    ...
    return { success: false, error: "Error inesperado al crear el empleado" }
  }
}
...
export async function getEmployees(onlyActive = true) {
  try {
    ...
    return { success: false, error: "Error inesperado al obtener empleados", data: [] }
  }
}
...
export async function getEmployee(employeeId: string) {
  try {
    ...
    return { success: false, error: "Error inesperado al obtener el empleado", data: null }
  }
}
...
export async function updateEmployee(employeeId: string, data: EmployeeFormData) {
  try {
    ...
    return { success: false, error: "Error inesperado al actualizar el empleado" }
  }
}
...
export async function deactivateEmployee(employeeId: string) {
  try {
    ...
    return { success: false, error: "Error inesperado al dar de baja al empleado" }
  }
}
